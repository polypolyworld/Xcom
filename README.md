...
兩平台風控壓力遠小於 IG，帳號健康時無需停發養號，邊發邊補真人信號即可。

### X（風險最低）
- Cookie 注入（`auth_token`/`ct0`）是常見會話方式，X 對自動化容忍度較高（實測單日連發 6 條未觸發風控）
- 新帳號建議補真人信號：關注 20–30 個相關帳號、偶爾點讚/轉發
- 頻率：單日發帖 ≤5 條，間隔 1 小時以上

### Facebook（中等風險，重點在登入方式）
- 主要風險不在發帖頻率，而在**每次都帳密登入**：從資料中心 IP 頻繁密碼登入易觸發 checkpoint（人臉/好友識別驗證）
